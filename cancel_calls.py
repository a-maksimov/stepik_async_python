...
async def call_company(data_item):
    try:
        await asyncio.wait_for(asyncio.sleep(data_item['call_time']), 6)
        print(f'Company {data_item["Name"]}: {data_item["Phone"]} дозвон успешен')
    except TimeoutError:
...
async def main():
    tasks = [call_company(data_item) for data_item in data]
