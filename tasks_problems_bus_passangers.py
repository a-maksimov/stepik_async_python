...
async def process_passenger(speed):
    await asyncio.sleep(speed)
    return f'{asyncio.current_task().get_name()} сел в автобус.'


async def main():
    tasks = [asyncio.create_task(process_passenger(passenger["Speed"]), name=passenger["Name"]) for passenger in passengers]
    try:
        await asyncio.wait_for(asyncio.gather(*tasks), 5)
    except TimeoutError:
        pass

    for task, passenger in zip(tasks, passengers):
