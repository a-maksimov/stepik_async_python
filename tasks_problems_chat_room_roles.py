...
async def check_message(msg):
    msg_text = msg["message"]
    role = asyncio.current_task().get_name()
    if role == 'admin':
        print(f"{role}: {msg_text}")

    elif role == 'moderator':
        for word in banned_words:
            if word in msg_text.lower():
                msg_text = msg_text.replace(word, "****")
        print(f"{role}: {msg_text}")

    elif role == 'student':
        for word in banned_words:
            if word in msg_text.lower():
                print(f"{role}: В сообщении есть запрещённое слово, сообщение скрыто")
                break
        else:
            print(f"{role}: {msg_text}")
...
async def main():
    tasks = [asyncio.create_task(check_message(msg), name=msg["role"]) for msg in messages]
