...
async def execute_subtask(task_name, duration):
    ...
    await asyncio.sleep(set_duration)
    return duration <= 5


async def execute_task(task_name, subtasks):
    subtasks = [execute_subtask(subtask['название'], subtask['время']) for subtask in subtasks]
...
async def main():
    tasks = [execute_task(task, tasks_dependencies[task]['этапы']) for task in tasks_dependencies]
